            ...
            return dbContext.Master_Player.Where(p => p.FK_VenueId == venueid && p.FK_StatusId == 1 && p.PK_PlayerId == (playerid != 0 ? playerid : p.PK_PlayerId))
                        ...
                        .Select(p =>
                          new PlayerModel
                          {
                              ...
                              StartTime = dbContext.Master_BatchTiming.Where(t=>t.FK_BatchId== p.playspobatcou.playspobatcou.playspobat.batch.PK_BatchId).ToList().FirstOrDefault().StartTime,
                              EndTime = dbContext.Master_BatchTiming.Where(t => t.FK_BatchId == p.playspobatcou.playspobatcou.playspobat.batch.PK_BatchId).ToList().FirstOrDefault().EndTime,
