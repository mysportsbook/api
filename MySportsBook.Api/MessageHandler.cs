...
namespace MySportsBook.Api
{
    ...
    public class MessageLoggingHandler : MessageHandler
    {
        ...
        {
            await Task.Run(() =>
               fileLogger.Log(string.Format("{0}-{1} - Request: {2}\r\n{3}", DateTime.Now.ToLongTimeString(), correlationId, requestInfo, Encoding.UTF8.GetString(message))));
        }
        ...
        {
            await Task.Run(() =>
                fileLogger.Log(string.Format("{0}-{1} - Response: {2}\r\n{3}", DateTime.Now.ToLongTimeString(), correlationId, requestInfo, Encoding.UTF8.GetString(message))));
        }
    }
}